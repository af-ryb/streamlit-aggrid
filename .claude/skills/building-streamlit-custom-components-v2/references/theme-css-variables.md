...
### Variable naming: `--st-*` prefix vs legacy unprefixed

Streamlit has two sets of CSS variable names:

- **`--st-*` prefixed** (modern): `--st-primary-color`, `--st-text-color`, `--st-background-color`, etc. These are the canonical names documented here.
- **Legacy unprefixed**: `--primary-color`, `--text-color`, `--background-color`, `--secondary-background-color`, `--font`. These are still available on the page `:root` and work in both `isolate_styles=True` and `isolate_styles=False` modes.

**Recommendation:** Use `--st-*` prefixed variables for new code. If you inherit an existing component that uses unprefixed names, they will continue to work.

### `isolate_styles` and CSS variable inheritance

- **`isolate_styles=True` (default):** Component is in a Shadow DOM. Streamlit injects `--st-*` variables into the shadow root. CSS custom properties inherit through shadow boundaries, so they are available.
- **`isolate_styles=False`:** Component is in the main DOM. All CSS variables on `:root` are available, including both `--st-*` and legacy unprefixed variables.

When using `isolate_styles=False`, you **must** scope your CSS selectors under a wrapper class to prevent style leakage:

```css
/* Good: scoped */
.my-component .label { color: var(--st-text-color); }

/* Bad: global leak */
.label { color: var(--st-text-color); }
```
