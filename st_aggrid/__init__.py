...
def _cast_date_columns_to_iso8601(dataframe: pd.DataFrame):
    ...
    for column, dtype in dataframe.dtypes.items():
        if dtype.kind == 'M':
            dataframe[column] = dataframe[column].apply(lambda s: s.isoformat())

    return dataframe


def __parse_row_data(data) -> None | tuple:
    """Internal method to process data from data_parameter"""
    if data is None:
        return None

    if isinstance(data, pd.DataFrame):
        # data = _cast_date_columns_to_iso8601(data)
        ...
        data['__pandas_index'] = [str(i) for i in range(data.shape[0])]
        frame_dtypes = dict(zip(data.columns, (t.kind for t in data.dtypes)))

        return json.loads(data.to_json(orient='records', date_format='iso')), frame_dtypes
...
def __parse_grid_options(gridOptions_parameter, data, default_column_parameters, unsafe_allow_jscode):
    ...
    if gridOptions_parameter is None and data is not None:
    ...
    if unsafe_allow_jscode:
        # NOTE: Streamlit should allow passing a class to its inner component dumps, this way any class could serialize on AgGrid call.
...
def AgGrid(
    ...
    columns_state: list[dict], optional
        List with columns definitions. Used to restore column state.
    ...
    key : typing.Any, optional
        Streamlit key argument. Check streamlit's documentation.
        Defaults to None.
    ...
    **default_column_parameters:
    ...
    response = AgGridReturn(data, gridOptions=gridOptions,
    ...
    if component_value:
        response.set_component_value(component_value)
