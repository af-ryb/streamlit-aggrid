...
class AgGridReturn(Mapping):
    ...
    def set_component_value(self, component_value):
        ...
        streamlit.session_state.ag_columns_state = self.columns_state
        streamlit.session_state.ag_grid_state = self.grid_state
        streamlit.session_state.ag_selected_rows = self.selected_rows_id

    @property
    def grid_response(self):
        """Raw response from component."""
        return self.__dict__.get("grid_response", {})
    ...
    def grid_state(self):
        """Gets the grid state.
        Typically used on initialState option. (https://ag-grid.com/javascript-data-grid//grid-options/#reference-miscellaneous-initialState)
        """
    ...
    def __process_vanilla_df_response(self, nodes, __try_to_convert_back_to_original_types):
        """Transformation of dataframe columns to original types"""
    ...
    def __get_data(self, onlySelected):
        ...
        if self.__component_value_set:
            ...
            reindex_ids = reindex_ids_map.get(DataReturnMode.FILTERED_AND_SORTED, None)

            if reindex_ids:
                reindex_ids = pd.Index(reindex_ids)

                if onlySelected:
                    return data.reindex(index=reindex_ids.intersection(data.index))
    ...
    def data_groups(self):
        """Returns grouped rows as a dictionary, where keys are tuples of groupby strings
        and values are pandas.DataFrame"""

        return self.__get_dataGroups(onlySelected=False)

    @property
    def selected_data_groups(self):
        """Returns selected rows as a dictionary where keys are tuples of grouped column names
        and values are pandas.DataFrame"""
